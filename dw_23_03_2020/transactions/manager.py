...
class TransactionManager:
    ...
    def calculate_transactions(self):
        while True:
            inp = input('Enter next transaction or type quit to stop: ')
            if inp == 'quit':
    ...
    def show_transactions(self, user_name=None):
        if not user_name:
            with open(str(input("Which user's transactions you want to see? ")) + '.pkl', 'rb') as file:
                trans = pickle.load(file).transactions
        else:
            trans = self.transactions
        for key in trans:
            print(f'{key}: {reduce(lambda x, y: x + y, tuple(trans[key]))}')
        print('Total: ' + str(reduce(lambda x, y: x + y, trans.values())))
